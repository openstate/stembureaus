...
class TestRemovingUserConnectedToOneGemeente(BaseTestClass):
  ...
  def setUp(self):
    ...
    self.user1 = add_test_user(self, self.gemeente, "testuser1@openstate.eu")
    self.user2 = add_test_user(self, self.gemeente, "testuser2@openstate.eu")
...
class TestRemovingUserConnectedToMultipleGemeenten(BaseTestClass):
  ...
  def setUp(self):
    ...
    from tests.utils import add_test_user
    self.user1 = add_test_user(self, self.gemeente1, "testuser1@openstate.eu")
    add_user_to_gemeente(self, self.user1, self.gemeente2)
    self.user2 = add_test_user(self, self.gemeente1, "testuser2@openstate.eu")
    self.user3 = add_test_user(self, self.gemeente2, "testuser3@openstate.eu")
